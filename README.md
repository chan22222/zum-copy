...
3. 끝 — 루트의 `Dockerfile`로 빌드되므로 **서비스가 하나만 생성**됩니다
   (workspaces를 감지해 client/server로 쪼개지지 않음). `PORT`는 Railway가
   자동 주입하고, 헬스체크(`/api/health`)는 `railway.toml`에 정의되어 있습니다.
   발급된 도메인이 곧 서비스 주소입니다.
